...
module.exports = {
  ...
  handleRequest: function(request, response){
    ...
    if(self.servingOnly){
       // serve directly
      return self.serveFile(original_filename, response); 
    }
  ...
  startServer: function(supplied_config){
    ...
    self.servingOnly = self.config.server.serving_only;

    self.generateInterval = (self.config.server.generate_interval || 10000);

    http.createServer(function(request, response){ 
      self.handleRequest(request, response);
    }).listen(parseInt(self.config.server.port));     

    console.log("Server is running on localhost:" + self.config.server.port);
  }
