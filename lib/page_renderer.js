...
module.exports = {
	...
	getHelpers: function(basepath, output_extension, options, callback) {
		...
		var get_helper_content = function(helper, helper_callback){
			helper.get(basepath, output_extension, options, function(err, helper_content, helper_options){
				if(!err){
		...
		var get_helper_callback = function(){
			if (cloned_helpers.length) {
				...
				return callback(null, collected_helper_content, collected_helper_options);	
			}	
		}
	...
	renderContent: function(request_path, output_extension, last_modified, options, callback) {
		...
		self.getHelpers(basepath, output_extension, options, function(err, helper_obj, helper_options) {
			...
			if (!err) {
				template_engine.setHelpers(helper_obj);
			}
		});
	},
