...
module.exports = {
				...
				if (_.include(compiler.input_extensions, template_extension)) {
					if (template.last_modified > last_modified) {
						...
						self.templates.readTemplate(template.full_path, function(err, template_output) {
							...
							return compiler.compile(template_output, compiler_callback);	
						});
					} else {
						return callback(null, { "body": null, "modified": false });	
					} 
	...
	renderContent: function(request_path, output_extension, last_modified, options, callback) {
		...
		template_engine.on("renderCanceled", function(err, status_code) {
			return callback(null, { "body": null, "modified": true, "options": { "header": { "status": status_code }, "log": { "message": err } } });	
		});
	...
	render: function(request_path, output_extension, last_modified, options, callback) {
		...
		var callback_handler = function(err, output) {
			if (!err) {
				if (_.has(output, "body") && _.has(output, 'modified')) {
					...
					return callback({ "body": null, "modified": true, "options": { "header": { "status": 500 }, "log": { "message": "[Error] Response should contain body and modified properties" } } });	
				}
			} else if(!err.ignore){
				return callback({ "body": null, "modified": true, "options": { "header": { "status": 500 }, "log": { "message": "[Error] " + err.message } } });	
			} else {
		...
		var rendering_steps = [
			function() { self.serveStatic((request_path + output_extension), last_modified, callback_handler) },		
