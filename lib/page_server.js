...
module.exports = {
	...
	prepareResponse: function(response, request_path, rendered_obj, file_extension){
		...
		if (rendered_obj.modified) {
			if (status_code === 200 && rendered_obj.body.length) {
				self.cacheStore.update(request_path, file_extension, rendered_obj.body, header, function(err, cache_obj) {
					if (err) {
    ...
    var request_path = parsed_url.pathname.replace(/\.\.\/|\/$/g, '');
		...
		var request_basename = ( request_path.indexOf(file_extension) > 0 ) ? request_path.substr(0, request_path.length - file_extension.length) : request_path;

		var options = {
			"query": parsed_url.query
		};

		self.cacheStore.lastUpdated(request_basename, file_extension, function(err, cache_last_updated){
			renderer.render(request_basename, file_extension, cache_last_updated, options, function(rendered_obj){
