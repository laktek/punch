...
module.exports = {
	...
	validatePublicCache: function(request, response, stat, callback) {
		...
		if (connect_utils.conditionalGET(request)) {
			...
			if (connect_utils.modified(request, null, headers)) {
				return callback();	
			}	else {
				return connect_utils.notModified(res);	
			}
		} else {
			return callback();	
		}
    ...
    var request_path = parsed_url.pathname.replace(/\.\.\/|\/$/g, '');
		...
		self.cacheStore.stat(request_basename, file_extension, function(err, stat) {
			renderer.render(request_basename, file_extension, stat.mtime, options, function(rendered_obj) {
