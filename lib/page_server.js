...
module.exports = {
	...
	getStatusPage: function(response, status_code, file_extension, header, request_options) {
		...
		self.cacheStore.get(status_code_path, file_extension, { "options": { "header": header } }, request_options, function(err, cache_obj){
			if(!err){
				...
				renderer.render(status_code_path, file_extension, null, request_options, function(rendered_obj) {
					...
					// alter rendered object's header with the current response header
					rendered_obj.options.header = header;

					var body_length = (rendered_obj.body && rendered_obj.body.length || 0);
					header["Content-Length"] = body_length;

					if (body_length) {
						self.cacheStore.update(status_code_path, file_extension, rendered_obj, request_options, function(err){
							if (err) {
								// we do nothing here.
								// maybe log the failure?
							}

							return self.sendResponse(response, status_code, rendered_obj.options.header, rendered_obj.body);
						});
					} else {
						return self.sendResponse(response, status_code, rendered_obj.options.header, "");
					}
				});
			}
		});
	...
	prepareRenderedResponse: function(response, request_basename, file_extension, rendered_obj, request_options) {
		...
		if (status_code === 200 && rendered_obj.body.length) {
			self.cacheStore.update(request_basename, file_extension, rendered_obj, request_options, function(err, cache_obj) {
				if (err) {
	...
	prepareCachedResponse: function(response, request_basename, file_extension, rendered_obj, request_options) {
		...
		self.cacheStore.get(request_basename, file_extension, rendered_obj, request_options, function(err, cache_obj) {
			return self.sendResponse(response, status_code, cache_obj.options.header, cache_obj.body);
		});
	},
