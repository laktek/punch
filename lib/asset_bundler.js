	...
	getBundle: function(request_basename, file_extension, request_options, callback) {
		...
		var prepare_bundle_and_update_cache = function() {
			return self.prepareBundle(bundle, file_extension, function(minified_output) {
				return self.cacheStore.update(bundle_name, file_extension, { "body": minified_output, "options": { "header": bundle_header } }, request_options, function(err, cache_obj){
		...
		self.cacheStore.stat(bundle_name, file_extension, request_options, function(err, stat) {
			...
			var check_for_modified = function() {
				if (cloned_bundle_files.length) {
					...
					return self.cacheStore.get(bundle_name, file_extension, { "options": { "header": bundle_header } }, request_options, function(err, cache_obj) {
						bundle_options.header = cache_obj.options.header;
