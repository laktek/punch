	...
	prepareBundle: function(bundle, output_extension, callback) {
		...
		var fetch_compile_and_minify = function() {
			if (cloned_bundle_templates.length) {
				...
				var current_template_basename = path_utils.getBasename(current_template_path, current_template_extension);

				if (self.isBundlePath(current_template_basename, current_template_extension)) {
					self.getBundle(current_template_basename, current_template_extension, {}, function(err, bundled_obj) {
						if (!err) {
							minified_outputs.push(bundled_obj.body);
						}
