...
module.exports = {
  ...
  setup: function(args) {
	// punch setup - Use the default template to create the site
	// punch setup boilerplate.zip - fetch the given template to create the site
	// punch setup --bare / punch setup -b - create a bare structure in current path
	// punch setup -b path - create a bare structure in given path
  ...
  server: function(args){
    if (args.length && args[0].match(/^\d+$/)) {
			...
			var overriden_port = parseInt(args[0]);
    } else {
      var config_path = args[0];
      var overriden_port = null;
    }
