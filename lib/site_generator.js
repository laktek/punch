...
module.exports = {
	...
	setup: function(config){
		...
		self.clearCache = config.generator.blank;
	...
	generate: function(callback){
		...
		var render_path_callback = function(rendered_path) {

			var generator_hooks_cloned = self.generatorHooks.slice(0);

			var move_to_next_or_end = function() {
			...
			var generator_hooks_callback = function() {
				if (rendered_path && generator_hooks_cloned.length) {
					var generator_hook = generator_hooks_cloned.pop();
					return generator_hook.run(rendered_path, generator_hooks_callback);	
				} else {
					return move_to_next_or_end();	
				}
			}

			return generator_hooks_callback();
		};
		
		var collect_and_generate = function() {
