...
module.exports = {
  ...
  fetchAndRender: function(template_path){
	...
	if(self.layout)
		...
		self.renderLayout(output, save_to_disk);
	}
	else{
  ...
  renderLayout: function(content,func){
	self.layout_renderer.setContent({"content":content});
	self.layout_renderer.afterRender = func;
	self.layout_renderer.setPartials({});
  },
